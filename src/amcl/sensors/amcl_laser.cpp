...
double compute_sample_weight(const AMCLLaserData *data, pf_sample_t *samples, const int sample_count)
{
  ...
  for (int j = 0; j < sample_count; j++)
  {
    ...
    // Take account of the laser pose relative to the robot
    pose = pf_vector_coord_add(laser_pose, pose);
    ...
    int step = (data->range_count - 1) / (max_beams - 1);
...
{
  const int sample_count = set->sample_count;
